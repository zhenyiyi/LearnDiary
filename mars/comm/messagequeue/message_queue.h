...
typedef uint64_t MessageQueue_t; // 消息队列
typedef boost::function<void ()> AsyncInvokeFunction; //异步调用函数

const MessageQueue_t KInvalidQueueID = 0; // 无效的队列ID。
...
struct MessageHandler_t {
    ...
    MessageQueue_t queue; // 消息队列
    unsigned int seq; // 序列ID
};
...
struct MessagePost_t {
    ...
    MessageHandler_t reg; // 消息处理
    unsigned int seq;// 序列ID
};

/**
 处理消息的target ,用于谁在处理这个消息
 */
struct MessageTitle_t {
    ...
    uintptr_t title; // 处理msg target的地址。
};


/**
 封装消息， 处理消息的taget(MessageTitle_t), 处理消息的 function
 */
struct Message {
...
struct MessageTiming {
    ...
    /**
     @param _timing TMessageTiming 类型
     @param _after 延迟几秒
     @param _period 时间间隔
     */
